...
def create_pr(repo: str, token: str, branch: str, base: str,
              title: str, body: str, issue_number: int) -> str:
    ...
    # Link PR to issue
    gh_request("POST", f"/repos/{repo}/issues/{result['number']}/labels", token,
               {"labels": ["taskaugen"]})
...
def main():
    ...
    try:
        pr_url = create_pr(
            repo, token, branch, base,
            title=f"[TaskAugen] {clean_title}",
            body=pr_body,
